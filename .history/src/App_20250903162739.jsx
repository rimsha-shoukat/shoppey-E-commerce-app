...
function App(){
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://dummyjson.com/products?limit=50&select=title,price,discountPercentage,rating,category,availabilityStatus,thumbnail');
        if (!response.ok) {
    ...
    <>
     <Routes>
      ...
      <Route path="/ProductDetail/:id" element={
            <Suspense fallback={<div>Loading...</div>}>
              <ProductDetail Products={ Productsproducts } />
            </Suspense>
          } />
