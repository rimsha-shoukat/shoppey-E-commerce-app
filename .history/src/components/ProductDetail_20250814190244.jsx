                ...
                <section className="flex flex-row items-center py-2 px-6 justify-between w-[100%] h-auto">
                 ...
                 <section className="grid grid-cols-2 max-[600px]:grid-cols-1 select-none items-center justify-center max-[600px]:w-[60%] max-[500px]:w-[70%] max-[400px]:w-[90%] w-[60%] max-[1140px]:w-[70%] max-[970px]:w-[80%] max-[800px]:w-[90%] h-auto border-2 border-gray-300 rounded-lg gap-[1rem] bg-[#f7ecd6]">
                    ...
                    <div className="cols-span-1 h-auto flex flex-col items-start justify-start p-6">
                        ...
                        <button onClick={() => AddedToCart(image.id); setAddedToCart(t)} className={`px-[2rem] py-[10px] rounded-md text-nowrap shadow-[0.5px_0.5px_2px] hover:shadow-[0.5px_0.5px_3px] hover:shadow-gray-800 hover:bg-gray-500 hover:text-white cursor-pointer`}>{addToCart ? 'Added to cart' : 'Add to cart'}</button>
                        <button onClick={() => SavedToCart(image.id); setAddedToCart(t)} className={`p-[10px] rounded-md shadow-[0.5px_0.5px_2px] hover:shadow-[0.5px_0.5px_3px] hover:shadow-gray-800 hover:bg-gray-500 hover:text-white cursor-pointer font-light text-[1.5rem]`}>{addToSave ? <FaBookmark /> : <FaRegBookmark /> }</button>
                        </div>
                    </div>
                 </section>
