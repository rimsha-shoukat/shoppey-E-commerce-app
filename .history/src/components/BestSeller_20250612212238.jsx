...
function Slider(props){
  ...
  return (
   ...
   { collection.slice(props.start, props.end).map((item) => {
    ...
    for(let i = 0; i < 5; i++){
      if(i < item.stars){
        Rating.push(<RiStarSFill className="text-yellow-400" key={`filled-${item.id}-${i}`} />)
      }else {
        Rating.push(<ImStarEmpty className="text-yellow-400 text-[0.8rem]" key={`empty-${item.id}-${i}`} />)
      }
    }
