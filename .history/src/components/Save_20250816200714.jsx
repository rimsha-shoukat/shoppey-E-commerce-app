...
const Saves = () => {
    ...
    return (
        <section className="w-[100%] h-auto flex flex-col items-center justify-center">
            {SaveItems.map((item) => (
                <div key={item.id} className="flex flex-row items-center justify-between w-[100%] p-4 border-b border-gray-300">
                    <img className="w-[8rem] h-[8rem] rounded-sm" src={item.img} alt={item.title} />
                    <div className="flex flex-col items-start justify-start">
                        <h1 className="text-[1.3rem] leading-5 font-bold">{item.title}</h1>
