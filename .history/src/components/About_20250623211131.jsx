    ...
    return(
            <div className="flex flex-row gap-4 items-center justify-between cursor-grab select-none scroll-smooth overflow-x-scroll overflow-y-hidden"
                style = {{
                    ...
                    {Users.map((user,i) => (
                        <div key={i} className="flex flex-col w-[18rem] h-auto flex-shrink-0 items-center p-4 justify-between gap-2 bg-[#eee2ca] rounded-md border-2 border-[#c8967d] font-sans">
                            <p className="font-semibold">{user.intro}</p>
                            <p className="text-sm text-justify">{user.review}</p>
                            <img  className="w-[3.5rem] h-[3.5rem] rounded-full shadow-md" src={user.img}/>
                            <h1>{user.name}</h1>
                            <h1>{user.rank}</h1>
                        </div>
                ))}
