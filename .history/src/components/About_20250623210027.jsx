...
function userSlider(){
    return(
        <>
            <div className="flex flex-row w-[90%] gap-4 items-center justify-between cursor-grab select-none scroll-smooth overflow-x-scroll overflow-y-hidden"
                style = {{
                    scrollbarWidth: 'none',
                    msOverflowStyle: 'none',
                }}>
                    {Users.map((user,i) => (
                        <div key={i} className="flex flex-col w-[15rem] h-auto flex-shrink-0 items-center justify-between gap-2 bg-[#eee2ca] rounded-md border-2 border-[#c8967d]">
                            <p className="font-semibold">{user.intro}</p>
                            <p className="">{user.review}</p>
                            <img  className="w-[2rem] h-[2rem] rounded-full" src={user.img}/>
                            <h1>{user.name}</h1>
                            <h1>{user.rank}</h1>
                        </div>
                    ))}
    ...
    return(
        <>
            <section className="flex flex-col w-[100%] h-auto overflow-hidden items-center max-[440px]:p-[0.8rem] justify-between gap-2 p-[2rem] font-serif">
                ...
                <div className="grid grid-rows-1 overflow-hidden mt-6 p-4">
                </div>
            </section>
        </>
    )
