  ...
  return (
    ...
    collection.map(product, key){
        <div className="w-auto h-auto bg-white leading-4">
          <img  className="w-[16rem] h-[13rem]" src={product.image} alt="" />
          <div className="p-[1rem] select-none">
            <span className="flex flex-row text-[1.2rem] text-[#ffff00]"><RiStarSFill /><RiStarSFill /><RiStarSFill /><RiStarSFill /><RiStarSFill /></span>
            <h1>{product.name}</h1>
            <p>Price: ${product.pri}</p>
          </div>
        </div>
    }
    </section>
