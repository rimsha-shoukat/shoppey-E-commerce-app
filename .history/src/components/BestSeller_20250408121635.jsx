   ...
   { collection.slice(props.start, props.end).map((item) => {
    ...
    return (
        <div key={item.id} className="w-auto h-auto bg-white leading-4">
          <img  className="w-[14rem] h-[14rem] drop-shadow-lg" src={item.image} alt="" />
          <div className="p-[0.5rem] select-none">
...
function BestSeller(){
  ...
  const handleClick = (index) => {
    ...
    return (
        <>
           <section className="flex flex-row max-[976px]:flex-col items-center justify-center gap-[2rem] w-[100%] h-auto py-[4rem] bg-[#251201]">
                ...
                <section className="flex flex-col gap-[1rem] w-[60%] overflow-hidden">
                ...
                <section className="max-[1185px]:justify-center flex flex-row items-start justify-start">
                  {Array.from({ length: Math.ceil(collection.length / itemsPerPage) }).map((_, index) => (
                      <button
                          ...
                          className={`w-[0.6rem] h-[0.6rem] cursor-pointer ml-1 rounded-full ${currentIndex === index * itemsPerPage ? 'bg-transparent' : 'bg-white'} border-white border-2`}
                      ></button>
                  ))}
                  </section>
                </section>
