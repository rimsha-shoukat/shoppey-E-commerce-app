...
function AllProducts({Products}) {
  ...
  useEffect( () => {
    ...
    if(param){
      ...
      } else if(['rating', 'discountPercentage', 'deal'].includes(param)){
        if(param === "rating"){
          Items = Items.filter(item => item.rating >= 3);
        } else if(param === "discountPercentage"){
          Items = Items.filter(item => item.discountPercentage > 1;
        } else if(param === "deal"){
      ...
      <div className="w-[100%] h-auto font-serif">
        ...
        { filterProducts.map((item) => (
            <Link key={item.id} to={`/ProductDetail/${item.id}`}>
              <img key={item.id}  className="w-full rounded-[1rem] mb-6 h-auto shadow-[0.1px_0.1px_0.1rem_#dd957a]" src={item.thumbnail} alt="products" loading="lazy" />
            </Link>
          ))}
        </section>
      </div>
