    ...
    return(
            <div className="flex flex-row gap-4 items-center justify-between cursor-grab select-none scroll-smooth overflow-x-scroll overflow-y-hidden"
                style = {{
                    ...
                    {Users.map((user,i) => (
                        <div key={i} className="flex flex-col w-[15rem] h-auto flex-shrink-0 items-center p-2 justify-between gap-2 bg-[#eee2ca] rounded-md border-2 border-[#c8967d]">
                            <p className="font-semibold text-justify">{user.intro}</p>
                            <p className="text-sm text-justif">{user.review}</p>
