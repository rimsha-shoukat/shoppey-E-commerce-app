    ...
    return(
        <>
            <section className="flex flex-col w-[100%] h-auto overflow-hidden items-center max-[440px]:p-[0.8rem] justify-between gap-2 p-[2rem] font-serif">
                ...
                <div className="flex flex-row w-[100%] p-[1rem] gap-4 items-center justify-between cursor-grab select-none scroll-smooth overflow-x-scroll overflow-y-hidden"
                style = {{
                    scrollbarWidth: 'none',
                    msOverflowStyle: 'none',
                }}>
                    {Users.map((user,i) => (
                        <div key={i} className="flex flex-col flex-shrink-0 items-center justify-between gap-2 bg-[#eee2ca] p-4 rounded-md border-2 border-[#c8967d]">
                            <p>{user.intro}</p>
                            <p>{user.review}</p>
                            <img src={user.img}/>
                            <h1>{user.name}</h1>
