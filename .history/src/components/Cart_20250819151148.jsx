...
const Carts = ({cartItemsList, setCartItemsList}) => {
    ...
    const [settingQ, setSettingQ] = useState(false);
    const [quantity, setQuantity] = useState(fals);
    
    return (
        <section className="w-[100%] h-auto flex flex-col items-center justify-center">
           {cartItemsList.map((item) => (
            <div key={item.id} className="w-[100%] flex flex-row items-center justify-between py-6 max-[350px]:py-2 gap-4 max-[350px]:gap-2 border-t-2 border-t-gray-400">
                        ...
                        <div className="flex flex-row items-start justify-start gap-[6rem] max-[600px]:gap-[3rem] max-[500px]:gap-2 max-[500px]:flex-col">
                            <div className="flex flex-col items-start justify-start gap-2">
                                <h1 className="font-bold text-[1.2rem] leading-4 max-[350px]:text-[0.95rem]">{item.name}</h1>
                            ...
                            <div className="flex flex-row items-center justify-center gap-[1rem] max-[350px]:gap-[0.75rem]">
                                <h1 className="font-bold max-[350px]:font-semibold max-[350px]:text-sm">${quantity * item.price}</h1>
                                ...
                                <FiMinus onClick={quantity >= 2 && (() => {setQuantity(quantity - 1); setSettingQ(true);})} className="cursor-pointer hover:text-gray-700 max-[350px]:text-sm"/>
                                    <h1 className="text-[1.2rem] max-[350px]:text-md">{settingQ ? quantity :  item.quantity}</h1>
                                    <FaPlus onClick={quantity <= 10 && (() => { setQuantity(quantity + 1); setSettingQ(true); })} className="cursor-pointer hover:text-gray-700 max-[350px]:text-sm"/>
                                </div>
        ...
        <>
            <div className="w-[100%] h-auto font-serif flex flex-col items-center justify-between select-none px-4 max-[350px]:px-2">
                ...
                <section className="w-[100%] h-auto flex flex-col items-center justify-center">
                    ...
                    <div className="w-[100%] flex flex-row max-[500px]:flex-col items-start max-[500px]:items-center max-[500px]:gap-4 justify-between py-6 border-t-2 border-t-gray-400">
                        ...
                        <div className="w-[100%] flex flex-col gap-2 items-start justify-start">
                            ...
                            <h1 className="text-[2rem] max-[350px]:text-[1rem]">Total: <span className="font-bold">$890.98</span></h1>
