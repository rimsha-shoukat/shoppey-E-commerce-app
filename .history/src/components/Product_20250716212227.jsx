  ...
  return (
      ...
      >
        {slide1.map((item) => (
          ...
          <div className="flex flex-col p-2 pt-2 bg-white w-[14.5rem]">
            <h1 className="font-semibold text-[1rem] over-flow-hidden">{item.name}</h1>
            <p>${item.price}</p>
          </div>
          </div>
        ))}
  ...
  return (
      ...
      >
        {slide2.map((item) => (
          ...
          <img className="w-[14.5rem] h-[13rem]" src={item.src} alt="demo" />
          <div className="flex flex-col p-2 pt-2 bg-white w-[14.5rem]">
            <h1 className="font-semibold text-[1rem] overflow-hidden">{item.name}</h1>
            <p>${item.price}</p>
          </div>
          </div>
        ))}
        ...
        <>
         <div className="w-[100%] h-auto p-[3rem] max-[430px]:p-[1rem] flex flex-col items-center justify-center font-serif select-none">
            ...
            <section className="flex flex-row items-center justify-center gap-4 max-[310px]:gap-2 leading-4 w-full h-8 max-[430px]:text-sm">
              {['NEW', 'WOMEN', 'MEN', 'TRENDS'].map((buttonName) => (
                  <button
                      key={buttonName}
