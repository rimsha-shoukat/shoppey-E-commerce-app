...
function AllProducts() {
  ...
  useEffect( () => {
    let Items = [...Products];
     ...
     if (param) {
      // Check if param is a category (men/women/kids)
      if (['men', 'women', 'kids'].includes(param)) {
        filtered = filtered.filter(item => item.category === param);
      } 
      // Check if param is a filter (new/trend)
      else if (['new', 'trend'].includes(param)) {
        filtered = filtered.filter(item => param === 'new' ? item.new : item.trend);
      }
    }
