   ...
   { collection.slice(props.start, props.end).map((item) => {
    ...
    return (
        <div key={item.id} className="w-auto h-auto bg-white">
          <img  className="w-[20rem] h-[18rem] drop-shadow-lg" src={item.image} alt="" />
...
function BestSeller(){
  ...
  const handleClick = (index) => {
    ...
    return (
        <>
           <section className="flex flex-row max-[920px]:flex-col items-center justify-center gap-[1.5rem] w-[100%] h-auto py-[5rem] bg-[#251201]">
                <article className="container flex flex-col items-start justify-start gap-[1.5rem] max-[920px]:pl-0 pl-[2rem] w-[35%] max-[1250px]:w-[40%] max-[920px]:w-[80%] text-white">
                    <h1 className="text-[clamp(2.8rem,4.5vw,5rem)] font-bold select-none leading-10">Best Seller Item</h1>
                    ...
                    <button className="hover:transition-all mt duration-700 delay-150 ease-in-out hover:text-[#8f5e36] font-bold cursor-pointer">SEE MORE</button>
                </article>
