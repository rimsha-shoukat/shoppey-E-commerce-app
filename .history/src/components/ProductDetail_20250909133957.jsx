...
function ProductDetail({ Products, user, setUser }) {
  ...
  const [cartItems, setCartItems] = useState(user?.cartItems || []);
    ...
    // Flags to track if current product is in cart/save lists
    const [addToCart, setAddToCart] = useState(false);
    const [addToSave, setAddToSave] = useState(false);
    // Sync local cart/save state when user prop changes (e.g. on login)
    useEffect(() => {
        setCartItems(user?.cartItems || []);
        setSaveItems(user?.saveItems || []);
    }, [user]);
    // Update addToCart and addToSave flags when id, cartItems or saveItems change
    useEffect(() => {
        setNum(1);
        setSelect('M');
