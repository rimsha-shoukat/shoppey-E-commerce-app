...
const Carts = ({cartItemsList, setCartItemsList}) => {
    ...
    function setItemQuantity(id, quantity) {
        const itemIndex = cartItemsList.findI(product => product.id === id);
        if (itemIndex !== -1) {
            const updatedItems = [...cartItemsList];
            ...
            <div key={item.id} className="w-[100%] flex flex-row items-center justify-between py-6 max-[350px]:py-2 gap-4 max-[350px]:gap-2 border-t-2 border-t-gray-400">
                        ...
                        <div className="flex flex-row items-start justify-start gap-[6rem] max-[600px]:gap-[3rem] max-[500px]:gap-2 max-[500px]:flex-col">
                            ...
                            <div className="flex flex-row items-center justify-center gap-[1rem] max-[350px]:gap-[0.75rem]">
                                <h1 className="font-bold max-[350px]:font-semibold max-[350px]:text-sm">${item.quantity * item.price}</h1>
                                ...
                                <FiMinus onClick={() => setItemQuantity(item.id, item.quantity - 1)} className="cursor-pointer hover:text-gray-700 max-[350px]:text-sm"/>
                                    <h1 className="text-[1.2rem] max-[350px]:text-md">{item.quantity}</h1>
                                <FaPlus onClick={() => setItemQuantity(item.id, item.quantity + 1)} className="cursor-pointer hover:text-gray-700 max-[350px]:text-sm"/>
                                </div>
