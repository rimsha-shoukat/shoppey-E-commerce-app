...
function Landing(){
    ...
    const[userView, setUserView] = useState(fals)
                        ...
                        <div className={`flex max-[710px]:flex-col text-sm max-[710px]:w-[85%] max-[710px]:h-auto min-[710px]:flex ${navView ? 'block' : 'hidden'} max-[710px]:right-[3rem] max-[710px]:absolute max-[710px]:bg-[#b48068] font-bold font-serif flex-row items-center justify-evenly max-[710px]:gap-[0rem] gap-[1.5rem]`}>
                            ...
                            </Link>
                            {
                                (localStorage.getItem('account')) ? (
                                    <button className="font-bold font-serif hover:transition-all duration-700 ease-in-out cursor-pointer max-[710px]:w-[100%] max-[710px]:hover:bg-black max-[710px]:hover:text-[#b48068] max-[710px]:py-[1rem] text-nowrap">{user.name}</button>
                                ) : (
                            ...
                            {
                                (localStorage.getItem('account')) ? (
                                    <div className="select-none font-bold font-serif absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-[#f1f1f1] shadow-[0.1px_0.1px_0.1rem_#dd957a] p-[2rem] rounded-md flex flex-col items-center justify-center gap-4">
                                        <h1 className="text-[1.5rem] text-[#b48068] leading-5">Hello! {user.name}</h1>
                                        <h1 className="text-[1.2rem]">{user.email}</h1>
                                        <button onClick={ ()=> { localStorage.setItem('account', JSON.stringify(false)); localStorage.setItem('currentUser', JSON.stringify(null)) }} className="cursor-pointer text-[1rem] bg-black text-[#b48068] border-2 border-black hover:text-black hover:bg-[#b48068] hover:transition-all duration-700 ease-in-out px-[1rem] py-[0.35rem]">Logout</button>
                                    </div>
                                ) : (null)
                            }
                        </div>
