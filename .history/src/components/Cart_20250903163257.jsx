           ...
           {cartItemsList.map((item) => (
            <div key={item.id} className="w-[100%] flex flex-row items-center justify-center py-6 max-[350px]:py-2 gap-4 border-t-2 border-t-gray-400">
                    <Link key={item.id} to={`/ProductDetail/${item.id}`}>
                        <img className="w-[6rem] h-[6rem] rounded-sm" src={item.thumbnai} alt="cartItem" loading="lazy" />
                    </Link>
...
function Cart(){
    const [cartItemsList, setCartItemsList] = useState(JSON.parse(localStorage.getItem('CartItems')) || []);
