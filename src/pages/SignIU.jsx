...
function SignIU() {
    ...
    return (
        <>
            <main className="relative flex flex-row  items-center justify-center w-[100%] h-[100vh] select-none ">
                {/* forms section */}
                <section className="relative max-[440px]:w-screen max-[800px]:h-auto w-[26rem] h-[30rem] bg-[#eee2ca] rounded-xl border-0 shadow-xl">
                    ...
                    <form onSubmit={handleSignIn} className={`${logIn ? 'block' : 'hidden'} w-[100%] h-[100%] flex flex-col items-center justify-center gap-[1rem] p-[4rem] max-[400px]:p-[1rem]`}>
                        ...
                        <button className="cursor-default text-sm text-gray-600 hover:text-red-600 mt-[1rem]">Forget your password!</button>
                        <button type="submit" className="cursor-default text-[1.5rem] shadow-md px-[3rem] py-[0.55rem] font-bold rounded-full bg-white/50">{loading ? "Loading>>>" : "SIGN IN"}</button>
                        <p className="text-gray-600 text-xs hidden max-[800px]:block">Or</p>
                        <p onClick={handleClick} className="hover:underline cursor-default hidden max-[800px]:block">signup</p>
                    </form>
                    ...
                    <form onSubmit={handleSignUp} className={`${logIn ? 'hidden' : 'block'} w-[100%] h-[100%] flex flex-col items-center justify-center gap-[1rem] p-[4rem] max-[400px]:p-[1rem]`}>
                        ...
                        <button type="submit" className="cursor-default text-[1.5rem] shadow-md px-[3rem] py-[0.55rem] font-bold rounded-full bg-white/50 mt-[1rem]">{loading ? "Loading>>>" : "SIGN UP"}</button>
                        <p className="text-gray-600 text-xs hidden max-[800px]:block">Or</p>
                        <p onClick={handleClick} className="hover:underline cursor-default hidden max-[800px]:block">signup</p>
                    </form>
                </section>
                ...
                <section className="relative right-[1rem] max-[800px]:right-0 max-[800px]:hidden max-[440px]:w-screen w-[24rem] h-[26rem] rounded-xl border-0 shadow-xl z-2">
                    ...
                    <div className={`${logIn ? 'block' : 'hidden'} relative w-[100%] h-[100%] flex flex-col items-center justify-center gap-2`}>
                        ...
                        <button onClick={handleClick} className="cursor-default text-[1.5rem] shadow-md px-[3rem] py-[0.55rem] font-bold rounded-full bg-white/50 mt-[3rem]">SIGN UP</button>
                    </div>
                    <div className={`${logIn ? 'hidden' : 'block'} relative w-[100%] h-[100%] flex flex-col items-center justify-center gap-2`}>
                        ...
                        <button onClick={handleClick} className="cursor-default text-[1.5rem] shadow-md px-[3rem] py-[0.55rem] font-bold rounded-full bg-white/50 mt-[3rem]">SIGN IN</button>
                    </div>
                </section>
            </main>
        </>
