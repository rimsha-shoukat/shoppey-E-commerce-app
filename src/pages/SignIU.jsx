      ...
      <main className="relative flex flex-row  items-center justify-center w-full mt-2 h-full select-none">
        ...
        <section className="relative max-[440px]:w-screen max-[800px]:h-auto w-[26rem] h-[30rem] bg-[#eee2ca] rounded-xl border-0 shadow-xl">
          ...
          <form
            ...
            className={`${logIn ? "block" : "hidden"} w-[100%] h-[100%] flex flex-col items-center justify-center gap-[1rem] p-[4rem] max-[400px]:p-[1rem]`}
          >
            <h1 className="text-[2rem] font-bold text-[#251201] mb-[2rem]">
              Sign In
            </h1>
            <input
              value={form.email}
              onChange={(e) => setForm({ ...form, email: e.target.value })}
              className="w-full bg-white/30 px-[0.5rem] py-[0.35rem] text-[1.1rem]"
              placeholder="Email"
              type="email"
              required
            />
            ...
            <p
              onClick={handleClick}
              className="hover:underline cursor-default hidden max-[800px]:block"
            >
              signup
            </p>
          </form>
          ...
          <form
            ...
            className={`${logIn ? "hidden" : "block"} w-[100%] h-[100%] flex flex-col items-center justify-center gap-[1rem] p-[4rem] max-[400px]:p-[1rem]`}
          >
            <h1 className="text-[2rem] font-bold text-[#251201] mb-[1rem]">
              Sign Up
            </h1>
            <input
              value={form.name}
        ...
        <section className="relative right-[1rem] max-[800px]:right-0 max-[800px]:hidden max-[440px]:w-screen w-[24rem] h-[26rem] rounded-xl border-0 shadow-xl z-2">
          <img
            className="absolute w-[100%] h-[100%] rounded-xl shadow-lg"
            src="assets/landing-bg.webp"
            alt="bgImage"
          />
          <div
            className={`${logIn ? "block" : "hidden"} relative w-[100%] h-[100%] flex flex-col items-center justify-center gap-2`}
          >
            <h1 className="font-bold text-[2.2rem]">Welcome Back!</h1>
            <p className="font-semibold text-center font-sans">
              Enter personal details <br /> to sign In your account
            </p>
            <button
              onClick={handleClick}
              className="cursor-default text-[1.5rem] shadow-md px-[3rem] py-[0.55rem] font-bold rounded-full bg-white/50 mt-[3rem]"
            >
              SIGN UP
            </button>
          </div>
          <div
            className={`${logIn ? "hidden" : "block"} relative w-[100%] h-[100%] flex flex-col items-center justify-center gap-2`}
          >
            <h1 className="font-bold text-[2.2rem]">Welcome!</h1>
            <p className="font-semibold text-center font-sans">
              Enter personal details <br /> to create your account
            </p>
            <button
              onClick={handleClick}
              className="cursor-default text-[1.5rem] shadow-md px-[3rem] py-[0.55rem] font-bold rounded-full bg-white/50 mt-[3rem]"
