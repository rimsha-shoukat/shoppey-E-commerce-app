export function Profile({ user, setUser, setUserView, userView }) {
        ...
        <>
            {
                user ? (
                    userView && <div className="w-1/2 select-none font-bold font-serif absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-[#f1f1f1] shadow-[0.1px_0.1px_0.1rem_#dd957a] p-[3rem] rounded-md flex flex-col items-start justify-center gap-4">
                        ...
                        <input disabled type="email" value="zarail@gmail.com" />
                        <div className="w-full flex flex-row items-center justify-between">
                            <input disabled type="number" value="03001234567" />
                        ...
                        <div className="flex flex-row items-center justify-between w-full">
                            <input disabled className="w-full" type="text" value="Masjid door zaroon Lahore, Pakistan" />
                            <button disabled className="cursor-pointer text-[0.8rem] text-[#b48068] hover:text-black hover:transition-all duration-700 ease-in-out">Add</button>
                        </div>
